    ...
    <div class="grow flex flex-col justify-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="ext in extensions" :key="ext.name">
                <div class="extension-card flex flex-col h-full bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                    <div class="p-5 grow">
                        <div class="flex items-center mb-3">
                            <span class="text-2xl leading-none" aria-hidden="true">{{ ext.icon }}</span>
                            <h3 class="font-display text-sm font-bold ml-2 text-gray-900 dark:text-white">{{ ext.name }}</h3>
                        </div>
                        <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">{{ ext.description }}</p>
                    </div>
                    <div class="px-5 pb-5">
                        <a
                            :href="ext.url"
